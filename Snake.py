...
#########################################################
    ...
    def __init__(self, root, n_obstacles, n_fruits, rows=25, cols=25, scale=20):
        ...
        self.snake_pixels = [
            Pixel(self.canvas, int(cols/2)-snake_dir[1]*3, int(rows/2)-snake_dir[0]*3, rows, cols, scale, 4, snake_dir),
            Pixel(self.canvas, int(cols/2)-snake_dir[1]*2, int(rows/2)-snake_dir[0]*2, rows, cols, scale, 4, snake_dir),
            Pixel(self.canvas, int(cols/2)-snake_dir[1]*1, int(rows/2)-snake_dir[0]*1, rows, cols, scale, 4, snake_dir),
            Pixel(self.canvas, int(cols/2)-snake_dir[1]*0, int(cols/2)-snake_dir[0]*0, rows, cols, scale, 5, snake_dir)            
        ]
    ...
    def next(self):
        ...
        if self.pixarr[self.snake_pixels[-1].i, self.snake_pixels[-1].j] == 3:
            self.fruits_eaten += 1
            self.n_fruits -= 1
            self.snake_pixels.insert(0, Pixel(self.canvas, self.snake_pixels[0].i-self.snake_pixels[0].vector[1], self.snake_pixels[0].j-self.snake_pixels[0].vector[0], self.rows, self.cols, self.scale, 4, self.snake_pixels[0].vector))
